...
# ---- Optional Multi-GEO helpers (no crash if missing in harmonizer.py)
...
if run:
    ...
    # ---- Load report for KPIs (just to possibly warn about PCA skip) ----
    report = {}
    try:
...
with tabs[5]:
    ...
    st.caption("Tip: counts = gene x sample table; meta = clinical/phenotype table. IDs should map to sample names (or provide candidate ID columns).")
    ...
    if run_multi:
        if not geo_rows:
            ...
            st.stop()
        if not run_pipeline_multi:
            st.error("Multi-GEO requires `run_pipeline_multi` in harmonizer.py. Please add it.")
            st.stop()
        try:
    ...
    if "multi_geo" in st.session_state and st.session_state.multi_geo:
        ...
        try:
            summary = summarize_multi_results(multi_res) if summarize_multi_results else {}
        except Exception:
            summary = {}
