    ...
    try:
        ...
        for gsm_name, gsm in gse.gsms.items():
            ...
            rows.append({
                ...
                "bare_id": gsm_name,
                "tissue_type": "; ".join([x for x in ch.get("characteristics_ch1", []) if
                                          "tissue" in x.lower()]) if "characteristics_ch1" in ch else "",
                "library_strategy": ", ".join(ch.get("library_strategy", [])) if "library_strategy" in ch else "",
                "extraction_protocol": ", ".join(
                    ch.get("extraction_protocol_ch1", [])) if "extraction_protocol_ch1" in ch else "",
            })
...
def _read_10x_minimal(path: str):
    ...
    try:
        ...
        # Resolve filenames
        mtx = os.path.join(mdir, 'matrix.mtx.gz') if os.path.exists(
            os.path.join(mdir, 'matrix.mtx.gz')) else os.path.join(mdir, 'matrix.mtx')
        feats = None
...
def read_expression_any(bytes_or_path, name_hint: Optional[str] = None, group_name: Optional[str] = None,
    ...
    def _read_excel(x):
        if is_pathlike:
            ...
            bio = _as_bytesio(x); bio.seek(0); df = pd.read_excel(bio, sheet_name=0, engine="openpyxl")
        return df
...
def detect_data_type_and_platform(X: pd.DataFrame) -> Tuple[str, str, Dict]:
    ...
    if has_ilumn or has_affy:
        platform = "Microarray (Illumina/Affy)";
...
def create_sample_qc_figures(raw_expr, expr_log2, meta, figdir: str) -> List[str]:
    ...
    plt.title("Per-sample Library Size (pre-log)");
    plt.xlabel("Samples");
    plt.ylabel("Sum of counts")
    _savefig(os.path.join(figdir, "qc_library_size.png"));
    paths.append(os.path.join(figdir, "qc_library_size.png"))
    z = zero_rate.values;
    plt.figure(figsize=(8, 4))
    plt.hist(z[np.isfinite(z)], bins=40, density=False)
    plt.title("Per-sample Zero Rate");
    plt.xlabel("Fraction zeros");
    plt.ylabel("Samples")
    _savefig(os.path.join(figdir, "qc_zero_rate_hist.png"));
    paths.append(os.path.join(figdir, "qc_zero_rate_hist.png"))
    return paths
...
def plot_housekeeping_stability(expr_log2, figdir: str) -> Optional[str]:
    ...
    p = os.path.join(figdir, "hk_cv.png");
    _savefig(p);
    return p
...
def plot_sex_marker_check(expr_log2, meta, figdir: str) -> Optional[str]:
    ...
    plt.xlabel("Male markers (avg log2)");
    plt.ylabel("Female markers (avg log2)")
    plt.title("Sex-marker Concordance (per sample)")
    p = os.path.join(figdir, "sex_marker_concordance.png");
    _savefig(p);
    return p
...
def create_basic_qc_figures(expr_log2, expr_z, expr_harmonized, meta, figdir: str) -> List[str]:
    ...
    plt.title("Expression Distribution: Pre vs Post (log2)");
    plt.xlabel("log2(Expression + 1)");
    plt.ylabel("Density");
    plt.legend()
    p = os.path.join(figdir, "dist_pre_vs_post_log2.png");
    _savefig(p);
    paths.append(p)

    plt.figure(figsize=(12, 6))
    arr = expr_z.values.ravel();
    arr = arr[np.isfinite(arr)]
    plt.hist(arr, bins=120, density=True)
    plt.title("Distribution of Z-scored Expression (All Samples)");
    plt.xlabel("Z-score");
    plt.ylabel("Density")
    p = os.path.join(figdir, "dist_zscore.png");
    _savefig(p);
    paths.append(p)
    ...
    plt.ylabel("Density");
    plt.legend()
    p = os.path.join(figdir, "group_density_post_log2.png");
    _savefig(p);
    paths.append(p)
    ...
    if group_vals:
        plt.figure(figsize=(12, 6));
        plt.boxplot(group_vals, labels=labels, showfliers=True)
        plt.title("Expression Distribution After Harmonization (log2)");
        plt.ylabel("log2(Expression + 1)")
        p = os.path.join(figdir, "boxplot_groups_harmonized_log2.png");
        _savefig(p);
        paths.append(p)
    ...
    if expr_harmonized.shape[1] <= 600:
        ...
        plt.imshow(C, aspect="auto", interpolation="nearest", vmin=-1, vmax=1)
        plt.colorbar(label="Correlation");
        plt.title("Sample Correlation Heatmap (post-harmonization)")
        plt.xlabel("Samples");
        plt.ylabel("Samples")
        p = os.path.join(figdir, "sample_correlation_heatmap.png");
        _savefig(p);
        paths.append(p)
    return paths
...
def create_enhanced_pca_plots(pca_df, pca_model, meta, output_dir, harmonization_mode):
    ...
    ax2.set_title("PCA: Batch Effects (Top 8)");
    ...
    xs = np.arange(1, n + 1);
    vals = pca_model.explained_variance_ratio_[:n];
    cum = np.cumsum(vals)
...
def nonlinear_embedding_plots(Xc, meta, figdir, harmonization_mode, make=True):
    ...
    if _HAVE_UMAP:
        try:
            ...
            Emb = reducer.fit_transform(Xp_embed);
            name = "umap"
        except Exception:
            pass
    if Emb is None and _HAVE_TSNE:
        try:
            ...
            Emb = tsne.fit_transform(Xp_embed);
            name = "tsne"
        except Exception:
            pass
    ...
    plt.title(f"{name.upper()} on ~{n_embed} PCs ({harmonization_mode})");
...
def volcano_and_ma_plots(de_df: pd.DataFrame, contrast_name: str, figdir: str):
    ...
    for c in de_df.columns:
        if c.startswith("mean_"):
            if means is None:
                means = de_df[c].values
            else:
                means = (means + de_df[c].values) / 2.0
    if means is None: means = np.zeros_like(log2fc)
    plt.figure(figsize=(8, 6));
    plt.scatter(means, log2fc, s=6, alpha=0.6)
    plt.xlabel("Avg log2 expression");
    plt.ylabel("log2FC");
    plt.title(f"MA Plot: {contrast_name}")
    _savefig(os.path.join(figdir, f"ma_{contrast_name}.png"))


def heatmap_top_de(expr_log2: pd.DataFrame, meta: pd.DataFrame, de_df: pd.DataFrame, contrast_name: str, figdir: str,
                   topn: int = 50):
    if de_df is None or de_df.empty: return
...
def run_pipeline(
    ...
    if metadata_batch_col is not None and metadata_batch_col in m_align.columns:
        ...
        notes["batch_source"] = "Batch inferred from sample names"
    ...
    row_mean = expr_log2.mean(axis=1);
    row_std = expr_log2.std(axis=1, ddof=1).replace(0, np.nan)
    ...
    # 9) Outliers
    outliers = detect_outliers(expr_log2);
    outliers.to_csv(os.path.join(OUTDIR, "outliers.tsv"), sep="\t")
...
def fetch_geo_as_datasets(accessions: list[str]) -> tuple[list[dict], pd.DataFrame, list[dict]]:
    ...
    for acc in accessions:
        ...
        if not re.fullmatch(r"GSE\d+", acc):
            failures.append((acc, "Invalid accession format"));
            continue
        ...
        try:
                ...
                for gsm_name, gsm in gse.gsms.items():
                    ...
                    rows.append({
                        ...
                        "library_strategy": ", ".join(
                            ch.get("library_strategy", [])) if "library_strategy" in ch else "",
                    })
            ...
            if expr is None or expr.empty:
                try:
                    ...
                    if mats:
                        ...
                        # Minimal meta
                        meta = pd.DataFrame(
                            {"sample": expr.columns, "group": "ALL", "bare_id": expr.columns}).set_index("sample")
                except Exception:
                    pass
            ...
            if expr is None or expr.empty:
                failures.append((acc,
                                 "Could not construct expression matrix (no series_matrix and no parsable supplementary/count tables)"))
                continue
...
def meta_analyze_disease_vs_control(runs: Dict[str, Dict], out_root: str, fdr_thresh: float = 0.10) -> Dict:
        ...
        for name, df in per.items():
            if g in df.index:
                ...
                if np.isfinite(lfc) and np.isfinite(p):
                    ...
                    zs.append(z);
                    signs.append(np.sign(lfc))
        if len(zs) >= 2:
            z_meta = _stouffer_meta(zs)
            p_meta = 2.0 * norm.sf(abs(z_meta))
            pos = np.sum(np.array(signs) > 0);
            neg = np.sum(np.array(signs) < 0)
    ...
    ups = meta_df[
        (meta_df["q_meta"] < fdr_thresh) & (meta_df["meta_log2FC_proxy"] > 0) & (meta_df["consistency"] >= 0.6)]
    ups.sort_values(["q_meta", "consistency"], ascending=[True, False]).to_csv(
        os.path.join(meta_dir, "upregulated_genes_meta.csv"))
    ...
    up_consistent = int(
        ((meta_df["q_meta"] < fdr_thresh) & (meta_df["meta_log2FC_proxy"] > 0) & (meta_df["consistency"] >= 0.6)).sum())
...
def run_pipeline_multi(
    ...
    for d in datasets:
        ...
        res = run_pipeline(
            ...
            metadata_group_cols=d.get("meta_group_cols") or ["group", "Group", "condition", "Condition", "phenotype",
                                                             "Phenotype"],
    ...
    if attempt_combine and len(exprs) >= 2:
        ...
        if n_common >= combine_minoverlap_genes:
            ...
            df_to_write.to_csv(expr_buf, sep="\t", index=False);
            expr_buf.seek(0)

            meta_buf = io.BytesIO()
            meta_all = meta_all.reset_index().rename(columns={"index": "sample"})
            meta_all.to_csv(meta_buf, sep="\t", index=False);
            meta_buf.seek(0)
